...
FULL_DATASET_PATH = r"E:\Regg Thesis\Dataset\Final_Dataset.csv"
GEOJSON_PATH = r'E:\Regg Thesis\Figures\bgd_admbnda_adm1_bbs_20201113.shp' 
...
def figure7_choropleth_map_overall():
    ...
    print(f"Starting Overall Choropleth Map generation...")
    
    # --- 1. Load and Prepare the Full Dataset ---
    try:
        df_full = pd.read_csv(FULL_DATASET_PATH)
        
        # --- KEY RENAME and Division Creation ---
        # Rename date and cases. 'errors=ignore' handles cases where the column is already renamed.
        df_full.rename(columns={
            'date': 'Date',            # Original date column
            'Patients': 'y_actual',    # Observed cases
        }, inplace=True, errors='ignore')
        
        # --- Division Column Reconstruction (Handles One-Hot Encoding) ---
        if 'Division' not in df_full.columns:
            division_cols = [c for c in df_full.columns if c.startswith('division_')]
            if division_cols:
                # Find the division name where the value is 1 (reconstructs 'Dhaka', 'Barishal', etc.)
                df_full['Division'] = df_full[division_cols].idxmax(axis=1).str.replace('division_', '')
            else:
                print("ERROR: Missing expected 'Division' column or 'division_' one-hot encoded columns.")
                return
    ...
    try:
        ...
        # Clean Division Names for merging (Crucial for GeoData compatibility)
        gdf['Division_Clean'] = (
        ...
        # 🚨 UPDATED COLORMAP: YlOrRd for consistency with error heatmap and risk visualization
        gdf.plot(column='y_actual', 
                  cmap='YlOrRd', # <--- THIS IS THE CHANGE
                  linewidth=0.8, 
                  ax=ax, 
                  edgecolor='0.5', 
                  legend=True,
        ...
        for idx, row in gdf.iterrows():
            if row['geometry'].is_valid and not row['geometry'].is_empty:
                ...
                # Use white text for the darkest region (Dhaka/high cases) to ensure readability
        ...
        # 6. Save and Close
        # Filename updated to reflect the new color scheme
        save_figure('Figure7_Choropleth_Map_Overall_YlOrRd_2022_2025')
        plt.close()
